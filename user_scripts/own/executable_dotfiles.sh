...
# Specific Files and Directories to Track (Can be absolute or relative to $HOME)
# Create this file and add paths like: .local/share/applications/steam.desktop
# Directories can be added like: .config/waybar
...
stage_files() {
    ...
    # Stage specific files from the tracking list
    if [[ -f "$TRACKED_LIST" ]]; then
        ...
        grep -vE '^\s*#|^\s*$' "$TRACKED_LIST" | while read -r item; do
            ...
            if [[ -e "$HOME/$item" ]]; then
                ...
                log_warn "Listed file/dir not found, skipping: $item"
            fi
        done
        ...
        log_warn "No file tracking list found at $TRACKED_LIST. You need to create this file to track items."
    fi

    # Always stage updates to files that are already in the git index
    dotgit add -u
}
...
restore_dotfiles() {
    ...
    if dotgit checkout "$GIT_BRANCH" 2>/dev/null; then
        ...
        # Foolproof method: Check exactly what files are in the repo,
        # and if they already exist in $HOME, move them out of the way.
        dotgit ls-tree -r "$GIT_BRANCH" --name-only | while read -r repo_file; do
            local abs_path="$HOME/$repo_file"
            if [[ -e "$abs_path" || -L "$abs_path" ]]; then # -e checks for files, directories, and symlinks
                mkdir -p "$(dirname "$CONFLICT_BACKUP_DIR/$repo_file")"
                mv "$abs_path" "$CONFLICT_BACKUP_DIR/$repo_file"
                log_info "Backed up: $repo_file"
            fi
        done
